...
class WSODMetaArch(model.DetectionModel):
  ...
  def _get_rpn_groundtruth_proposals(self,
                                     preprocessed_inputs,
                                     height=None,
                                     width=None):
    ...
    Returns:
      proposals: A 3-D tensor of shape [batch, num_proposals, 4] representing the
        proposals before applying non-maximum suppression.
      unnormalized_proposals: A 3-D tensor of shape [batch, num_proposals, 4] 
        representing the proposals before applying non-maximum suppression.
    """
    method = self._proposal_prediction

    (batch, input_height, input_width,
     _) = shape_utils.combined_static_and_dynamic_shape(preprocessed_inputs)
    ...
    # Use Faster-RCNN anchors as proposals.

    if method.startswith('frcnn'):
      unnormalized_proposals = tf.tile(
          tf.expand_dims(self._anchors.get(), 0), [batch, 1, 1])
      proposals = tf.stack([
          box_list_ops.to_normalized_coordinates(
              box_list.BoxList(boxes), height, width).get()
          for i, boxes in enumerate(tf.unstack(unnormalized_proposals, axis=0))
      ],
                           axis=0)
      return proposals, unnormalized_proposals

    # Use boxes generated by EdgeBoxes as proposals.

    elif method.startswith('edge_boxes'):
      edge_boxes_inputs = (preprocessed_inputs + 1.0) * 255.0 / 2.0
      (_, proposals) = imgproc.get_edge_boxes(
          edge_boxes_inputs,
          self._edge_detection,
          self._edge_boxes,
          max_num_boxes=self._edge_boxes_max_num_boxes)
      unnormalized_proposals = tf.stack([
          box_list_ops.to_absolute_coordinates(
              box_list.BoxList(boxes), height, width).get()
          for i, boxes in enumerate(tf.unstack(proposals, axis=0))
      ],
                                        axis=0)
      return proposals, unnormalized_proposals
  ...
  def _visualize_rpn_groundtruth_proposals(
      self, preprocessed_inputs, class_activation_maps, proposals,
      proposal_scores, proposal_per_channel_scores, vocabulary_list):
    """Visualizes rpn groundtruth.
    ...
    Args:
      ...
      proposals: A 3-D tensor of shape [batch, num_proposals, 4] representing the
        proposals before applying non-maximum suppression.
      proposal_scores: A 2-D tensor of shape [batch, num_proposals] representing
        the confidence score of each proposal.
      proposal_per_channel_scores: a [batch, num_proposals, 1 + num_classes] 
        float tensor.
      vocabulary_list: a [1 + num_classes] string tensor.
    """
    # Normalize the `proposal_per_channel_scores`.
    min_v = 0
    max_v = tf.reduce_max(
        proposal_per_channel_scores[:, :, 1:], axis=[1, 2], keepdims=True)
    normalized_scores = (proposal_per_channel_scores[:, :, 1:] - min_v) / (
        tf.maximum(1e-20, max_v - min_v))
    proposal_per_channel_scores = tf.concat(
        [proposal_per_channel_scores[:, :, :1], normalized_scores], axis=2)

    with tf.name_scope("visualize_rpn_groundtruth_proposals"):
      ...
      for i, x in enumerate(tf.unstack(class_activation_maps, axis=-1)):
        ...
        image_with_boxes = tf.cond(
            tf.logical_or(
                tf.equal(i, 0),
                tf.greater(
                    tf.reduce_max(proposal_per_channel_scores[:, :, i]), 0.5)),
  ...
  def provide_wsod_rpn_groundtruth(self, preprocessed_inputs,
    ...
    edb_inputs = cam_inputs = tf.image.resize_images(
        preprocessed_inputs, [cam_input_size, cam_input_size])
    if edb_input_size != cam_input_size:
      edb_inputs = tf.image.resize_images(preprocessed_inputs,
                                          [edb_input_size, edb_input_size])

    # Compute the class activation maps.

    with tf.name_scope('CalculateClassActivationMaps'):
      (image_importance_score,
       class_activation_maps) = self._get_class_activation_maps(
           cam_inputs, vocabulary_list)
      class_activation_maps = tf.concat(
          [image_importance_score, class_activation_maps], axis=-1)

    # Compute the proposals and the scores associated.
    #   proposals shape=[batch, num_proposals, 4].
    #   proposal_scores shape=[batch, num_proposals].
    #   proposal_per_channel_scores shape=[batch, num_proposals, 1 + num_classes].

    with tf.name_scope('CalculateRPNGroundtruthProposals'):
      proposals, unnormalized_proposals = self._get_rpn_groundtruth_proposals(
          edb_inputs, height=cam_input_size, width=cam_input_size)

      proposal_per_channel_scores = self._proposal_saliency_fn(
          class_activation_maps, tf.cast(unnormalized_proposals, tf.int64))
      ...
      if self._proposal_prediction == 'edge_boxes_ori_scores':
        ...
        proposal_scores = utils.masked_maximum(
            data=proposal_per_channel_scores[:, :, 1:],
            mask=tf.expand_dims(
    ...
    self._visualize_rpn_groundtruth_proposals(
        preprocessed_inputs, class_activation_maps, proposals, proposal_scores,
        proposal_per_channel_scores, vocabulary_list)
  ...
  def _postprocess_box_classifier(self,
    ...
    wsod_detection_classes = tf.argmax(
        nmsed_additional_fields['wsod_classes'], axis=-1)
    if FLAGS.coco_eval_on_voc:
      score_tensors = []
      for i in [
          5, 2, 16, 9, 44, 6, 3, 17, 62, 21, 67, 18, 19, 4, 1, 64, 20, 63, 7, 72
      ]:
        score_tensors.append(
            nmsed_additional_fields['wsod_classes'][:, :, i - 1])
      wsod_detection_classes = tf.argmax(
          tf.stack(score_tensors, axis=-1), axis=-1)
    ...
    detections = {
        ...
        nmsed_scores,
        'detection_softmax_scores':
        nmsed_additional_fields['wsod_classes'],
        #fields.DetectionResultFields.detection_classes: nmsed_classes,
        fields.DetectionResultFields.detection_classes:
        wsod_detection_classes,
        fields.DetectionResultFields.num_detections:
  ...
  def loss(self, prediction_dict, true_image_shapes, scope=None):
    ...
    (wsod_groundtruth_boxes,
     wsod_groundtruth_box_scores) = self.provide_wsod_rpn_groundtruth(
         prediction_dict['preprocessed_inputs'], true_image_shapes)
  ...
  def _get_class_activation_maps(self, preprocessed_inputs, vocabulary_list):
    ...
    Returns:
      image_importance_score: a 4-D tensor of shape [batch, height, width, 1] 
        representing image saliency.
      class_activation_maps: a 4-D tensor of shape [batch, height, width, 
        num_classes] representing class-aware score map.
    """
    preprocessed_inputs = (preprocessed_inputs + 1.0) * 255.0 / 2.0

    with tf.variable_scope('wsod'):
      prediction_dict = self._saliency_model.build_prediction(
          examples={
              'image': preprocessed_inputs,
              'category_strings': vocabulary_list
          },
          prediction_task='image_score_map')

      (image_importance_score,
       class_activation_maps) = (prediction_dict['image_saliency'],
    ...
    return (tf.stop_gradient(image_importance_score),
            tf.stop_gradient(class_activation_maps))
